...
방한 외국인 관광객 구매 추천 제품 선정 (Top 15).

**후보 풀** = OliveYoung(한국 인기 ∪ 글로벌 주문) ∪ Sephora(리뷰 활성 제품)
- OY 랭킹만으론 AP의 한국 랭킹 점유가 낮아 후보가 빈약 → **미국 현지(Sephora) 인기까지 포함**해 방한 동선을 폭넓게 반영

**Pick Score (2026-06 개편 — Sephora 포함 + SNS 강화)**
```
pick_score = 0.35 × SNS 트렌드 강도 + 0.35 × VOC 긍정도 + 0.30 × 리테일 인기
```
- **SNS 트렌드 강도**: 연결된 트렌드 키워드의 z_score·momentum 정규화 블렌드 `max(0.6·min(1,z/10) + 0.4·min(1,momentum/3))` — "지금 해외에서 뜨는 정도"
- **VOC 긍정도**: 리뷰 만족도 (사도 실패하지 않을 제품)
- **리테일 인기** = OY 한국 랭킹 · OY 글로벌 랭킹 · Sephora 인기 중 **최고값**
  - OY 랭킹: Top 100 고정 기준 `max(0, 1 − (순위 − 1)/100)`
  - **Sephora 인기**: 랭킹 접근 불가 → 리뷰로 대리. **리뷰 유입 속도(velocity) 우선**, 없으면 **누적 리뷰 수 log 정규화 × 0.6 디스카운트**(스테디셀러 편향 완화)

> Sephora의 velocity는 연속 주차(W26+)가 쌓일수록 정확해진다. 초기에는 누적 리뷰 log 보조값이 주로 쓰임.

> **OY Top in Korea 스크래퍼 수정(2026-06):** "Top in Korea" 탭이 클래스 없는 `<a>`라 기존 클래스 셀렉터(`a.btn` 등)로 클릭 실패 → 기본 페이지를 잘못 수집하던 버그를 `a:has-text("Top in Korea")` 텍스트 셀렉터로 수정.
...
- **`StartWhenAvailable=True`(놓치면 따라잡기) 활성화 필수** — 예약 시각에 PC가 꺼져 있어도, 다음에 PC를 켜면 밀린 작업을 따라잡아 1회 실행. (이 설정이 꺼져 있으면 그냥 다음 주로 건너뜀)
